...
- **Rate limiting**: login (5/min), invites (10/hr), leave applications (20/hr), and accept-invite (per-token, effectively unbrute-forceable given 32-byte random tokens) are all rate-limited via `slowapi`. The limiter's key function reads the real client IP from `X-Forwarded-For` (set by Nginx) instead of `request.client.host`, which behind a reverse proxy would otherwise always resolve to Nginx's own container address — a subtle bug that would have put every real user into the same shared rate-limit bucket, making the whole rate-limiting story cosmetic in the Docker deployment
